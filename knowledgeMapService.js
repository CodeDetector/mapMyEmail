...
class KnowledgeMapService {
    ...
    async _rebuildMap(km) {
        ...
        try {
            // 1. Resolve employee name — email repo may not have getEmployeeById
            let empName = `Employee-${km.employee_id}`;
            try {
                const employees = await supabaseService.getEmployees();
                const match = employees.find(e => String(e.id) === String(km.employee_id));
